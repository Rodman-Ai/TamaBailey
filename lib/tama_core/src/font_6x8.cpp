...
// 6x8 font. 5 columns of glyph data + 1 trailing blank column for spacing.
// Each column byte is read with bit 0 = top pixel, bit 6 = bottom pixel
// of the visible 5x7 glyph (bit 7 left clear).
//
// Indexed by (ch - 0x20). Characters 0x20..0x7E are covered; anything
// outside the range or unsupported renders as a filled square.
const uint8_t kFont6x8[96][6] = {
  ...
  /* 0x7F     */ {0x7F, 0x41, 0x41, 0x41, 0x7F, 0x00},
};

int text_width(const char* s, int scale) {
  int n = 0;
  while (s && *s++) ++n;
  return n * kFontWidth * scale;
}
